...
function contactBlock(contact) {
  ...
  if (contact.interactions?.length) {
    ...
    // Include the full text of the few most recent messages actually sent,
    // so follow-ups build on what was said rather than repeating it.
    let fullTextBudget = 3;
    let hasSentMessages = false;
    for (const i of contact.interactions) {
      lines.push(`- [${i.date}] (${i.channel}) ${i.summary}`);
      if (i.message && fullTextBudget > 0) {
        fullTextBudget--;
        hasSentMessages = true;
        const text = i.message.length > 1200 ? i.message.slice(0, 1200) + '…' : i.message;
        lines.push(`  Full text of that message:\n  ${text.replaceAll('\n', '\n  ')}`);
      }
    }
    if (hasSentMessages) {
      lines.push('', 'Do not repeat points already made in previously sent messages — reference or build on them instead.');
    }
  } else {
